...
hijo1 suma los enteros desde el inicio hasta la mitad del vector
hijo2 suma los enteros desde la mitad hasta el final del vector.


*/

void error(char *msg) {  //para imprimir un mensaje de error y salir del programa si ocurre un error durante la ejecución.
    perror(msg); 
    exit(-1);
}

int LeerNumeros(char *filename, int **vec) {  //lee un archivo de texto que contiene números, almacena esos números en un vector dinámico y devuelve la cantidad total de números leídos.
    ...
    *vec = (int *)calloc(totalNum, sizeof(int));  //Tamaño totalNum y sizeof(int) tamaño en bytes de cada elemento 
